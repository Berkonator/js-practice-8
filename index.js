...
function getUsersFromDatabase(array) {
  ...
  let a = Array.from(userRecords, obj => ({...obj, name: obj.name.toUpperCase()}));
  return a;
...
function findElementIndexes(arr, element) {
  ...
  if(Array.isArray(arr)){
    if(arr.lastIndexOf(element) != arr.indexOf(element)){
      arr2.push(arr.indexOf(element), arr.lastIndexOf(element));
    }
    return arr2;
  };
  return arr2;
...
function customFill(arr, value, start = 0, end = arr.length) {
  if(Array.isArray(arr) && (start && end <= arr.length) && (start && end >=0)){
